...
def main():  # 定义主函数，包含服务器初始化和运行逻辑
    ...
    # 数据库 schema 定义天气数据表的SQL schema字符串，用于Prompt上下文
...
当前日期: {current_date} (Asia/Shanghai)
    ...
    """
    Agent Card 的作用：
    这是 Agent 的身份证，其他 Agent 通过它了解：
    我是谁：Weather Query Assistant
    我能做什么：天气查询
    怎么调用我：http://localhost:5005
    我的能力：支持流式、有记忆
    """
    # 天气查询服务器类
    class WeatherQueryServer(A2AServer):  # 定义天气查询服务器类，继承自A2AServer
        ...
        def generate_sql_query(self, conversation: str) -> dict:  # 定义生成SQL查询方法，输入对话历史，返回字典
            ...
            获取当前日期（Asia/Shanghai 时区）
                ↓
            创建 LangChain 链：Prompt + LLM
                ↓
            调用 LLM 生成输出
                ↓
            判断输出格式？
                ├─ 以"{"开头 → 解析为 JSON（需要追问）
                │   返回：{"status": "input_required", "message": "..."}
                └─ 否则 → 视为 SQL
                    返回：{"status": "sql", "sql": "..."}
                ↓
            异常处理 → 返回追问 JSON
        ...
        def handle_task(self, task):  # 定义处理任务方法，输入任务对象
            ...
            try:  # 开始异常捕获块
                ...
                if gen_result.get("status") == "input_required":  # 检查是否需要追问
                    # 追问逻辑，这里是指在无法正常生成sql时，
                    task.status = TaskStatus(state=TaskState.INPUT_REQUIRED,
                                             message={"role": "agent",
                                                      "content": {
                                                          "text": gen_result["message"]}})  # 设置任务状态为输入所需，添加追问消息
                    return task  # 返回任务
                ...
                try:  # 开始循环管理块
                    ...
                    weather_result = loop.run_until_complete(client.call_tool("query_weather",
                                                                              sql=sql_query))  # 异步调用MCP工具，传入SQL
                finally:  # 确保循环关闭
                    loop.close()  # 关闭事件循环
            ...
            SmartVoyage 选择异步的原因：
            多用户并发：可能同时有多个用户查询
            网络 IO 密集：大量 HTTP 请求等待响应
            符合标准：MCP 协议官方推荐使用异步客户端
            为什么不直接用 await？
            # ❌ 错误：handle_task 不是 async 方法
            def handle_task(self, task):
                result = await client.call_tool()  # SyntaxError!
            """
            """
            # 成功情况
            return {
                ...
                "artifacts": [{
                    "parts": [{
                        "type": "text",
                        "text": "北京 2025-08-01: 晴，温度 25-32°C..."
                    }]
                }]
            }
            ...
            return {
                ...
                "artifacts": [{
                    "parts": [{
                        "type": "text",
                        "text": "查询失败：Connection refused 请重试或提供更多细节。"
                    }]
                }]
            }
