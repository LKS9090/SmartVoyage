...
# 为什么需要：数据库返回的结果包含这些类型，必须转换为 JSON 兼容格式
...
class TicketService:  # 定义票务服务类，封装数据库操作逻辑
    ...
    def execute_query(self, sql: str) -> str:  # 定义执行SQL查询方法，输入SQL字符串，返回JSON字符串
        try:  # 开始异常捕获块，确保查询鲁棒性
            ...
            return json.dumps({"status": "success", "data": results} if results else {"status": "no_data", "message": "未找到票务数据，请确认查询条件。"}, cls=DateEncoder, ensure_ascii=False)  # 序列化为JSON，如果有结果返回success，否则no_data；使用DateEncoder，非ASCII不转义
        except Exception as e:  # 捕获任何异常
            logger.error(f"票务查询错误: {str(e)}")  # 记录错误日志
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)  # 返回错误JSON响应
    """
    输入 SQL 字符串
        ↓
    创建字典游标（dictionary=True）
        ↓
    执行 SQL 语句
        ↓
    获取所有结果（fetchall）
    ...
    关闭游标
        ↓
    遍历结果 → 格式化特殊类型（date/datetime/Decimal）
        ↓
    判断结果是否为空？
        ├─ 有数据 → {"status": "success", "data": [...]}
        └─ 无数据 → {"status": "no_data", "message": "..."}
        ↓
    使用 DateEncoder 序列化
        ↓
    ...
    def default_encoder(self, obj):  # 定义默认编码器方法，用于格式化单个对象
        ...
        return obj  # 返回原对象
    """
    DateEncoder：用于 json.dumps() 的批量序列化
    default_encoder：用于单个对象的格式化（在结果遍历时调用）
    冗余设计？： 是的，这两处功能重复。可以优化为只保留一个。
    """
# 创建票务MCP服务器
def create_ticket_mcp_server():  # 定义创建票务MCP服务器的主函数
    ticket_mcp = FastMCP(  # 创建FastMCP实例
        name="TicketTools",  # 设置服务器名称
        description="票务查询工具，基于 train_tickets, flight_tickets, concert_tickets 表。只支持查询。",  # 设置服务器描述
        version="1.0.0"  # 设置版本号
    )  # MCP实例创建结束

    service = TicketService()  # 实例化票务服务对象

    @ticket_mcp.tool(  # 使用装饰器注册工具到MCP
        name="query_tickets",  # 工具名称
        description="查询票务数据，输入 SQL，如 'SELECT * FROM train_tickets WHERE departure_city = \"北京\" AND arrival_city = \"上海\"'"  # 工具描述，包括示例SQL
    )  # 工具注册参数结束
    def query_tickets(sql: str) -> str:  # 定义工具函数，输入SQL，返回JSON字符串
        logger.info(f"执行票务查询: {sql}")  # 记录查询SQL的INFO日志
        return service.execute_query(sql)  # 调用服务执行查询并返回结果

    # 打印服务器信息
    logger.info("=== 票务MCP服务器信息 ===")  # 记录服务器信息分隔线
    logger.info(f"名称: {ticket_mcp.name}")  # 记录服务器名称
    logger.info(f"描述: {ticket_mcp.description}")  # 记录服务器描述
    tools = ticket_mcp.get_tools()  # 获取所有注册工具列表
    for tool in tools:  # 遍历每个工具
        logger.info(f"- {tool['name']}: {tool['description']}")  # 记录工具名称和描述

    # 运行服务器
    port = 6002  # 设置服务器端口
    app = create_fastapi_app(ticket_mcp)  # 基于MCP创建FastAPI应用
    logger.info(f"启动票务MCP服务器于 http://localhost:{port}")  # 记录启动信息
    uvicorn.run(app, host="0.0.0.0", port=port)  # 启动Uvicorn服务器，绑定所有IP，指定端口

if __name__ == "__main__":  # 检查是否为直接运行模块
    create_ticket_mcp_server()  # 调用创建服务器函数